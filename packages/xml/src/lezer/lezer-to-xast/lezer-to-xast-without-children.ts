...
/**
 * Convert a `lezer` `SyntaxNode` into a `XastNode` if possible.
 * `null` is returned if there is no corresponding `Xast` node.
 */
export function lezerNodeToXastNodeWithoutChildren(
    node: SyntaxNode,
    source: string,
    ...
    switch (name) {
        ...
        case "Element": {
            ...
            for (const attrTag of openTag.getChildren("Attribute")) {
                ...
                attributes[extractContent(attrName, source)] =
